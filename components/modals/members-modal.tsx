...
export const MembersModal = () => {
  ...
  const [state, setState] = useState({ loadingId: "" });
        ...
        <ScrollArea className="mt-8 max-h-[420px] pr-6">
          {server?.members?.map((member) => (
            <div key={member.profileId} className="flex items-center gap-x-2 mb-6">
              ...
              {server.profileId !== member.profileId && state.loadingId !== member.id && (
              ...
              {state.loadingId === member.id && <Loader2 className="animate-spin text-zinc-500 ml-auto w-4 h-4" />}
            </div>
          ))}
        </ScrollArea>
      </DialogContent>
